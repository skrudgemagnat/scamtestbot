...
admin_id = 1145960734
# токен от бота
token = '993898138:AAF8Usl4bFqJlWijCiFtbcFBAqsdxIS3PPM'
#=====================
...
def add_item(message):
	try:
		if message.from_user.id == admin_id:
			conn_i = sqlite3.connect('items.sqlite3')
			cursor_i = conn_i.cursor()
			cursor_i.execute("INSERT INTO items VALUES ('"+str(random.randint(1111, 9999))+"', '"+str(message.text.replace('/add_item ', '').split(' | ')[0])+"', '"+str(message.text.replace('/add_item ', '').split(' | ')[1])+"')")
			conn_i.commit()
